...
name: smoke-test
description: |-
  Launch the app and hands-on verify that it works by interacting with it. Falls back to an existing integration test suite when there is no interactive surface in scope. Use when the user asks to 'smoke test', 'test it manually', 'verify it works', 'try it out', 'run a smoke test', 'check it in...
...
Fallback when Step 3 routed here because nothing was interactive. Run the discovered target. Tail output in a background shell for long-running suites so failures surface as they happen.
