REM
REM Sets up the Batch Repository User.
REM 
...
REM    User
REM 3: Creates the Batch Repository User if required and grants privileges
REM 4: logs on as the Batch Repository User
REM 5: Installs the EUL4_BATCH_REPOSITORY package
REM 6: Grants access to EUL4_BATCH_REPOSITORY
REM 7: Grants access for Batch Repository User to EUL
REM
REM
...
set termout on
PROMPT
PROMPT Discoverer Batch Repository User Installation Script
PROMPT ====================================================
...
PROMPT 2..Asks you to enter the name and password of the Batch Repository User.
PROMPT ...If the user does not exist, it will be created
PROMPT 3..Grants privileges to the Batch Repository User
PROMPT 4..Create the package EUL4_BATCH_REPOSITORY
PROMPT 5..Grants execute on EUL4_BATCH_REPOSITORY to PUBLIC
PROMPT 6..Grants access for Batch Repository User to EUL   
PROMPT
...
REM ------------------------------------------------------------------
REM Prompt for Batch Repository User to be installed
REM

PROMPT
PROMPT Enter details of the new Batch Repository User to be created
PROMPT 

accept Batch_User prompt 'Batch Repository Username : '
...
UNION
select 'prompt Batch Repository User being created'
from dual
where NOT exists
(select username from all_users
 where username = UPPER('&Batch_User'))
UNION
select 'prompt Batch Repository User already exists'
from dual
where exists
(select username from all_users
 where username = UPPER('&Batch_User'))
...
PROMPT for the Batch Repository User.  If you enter the
...
PROMPT
PROMPT Batch Repository Owner - &Batch_User
...
REM ------------------------------------------------------------------
REM Grant privileges to the Batch Repository User
REM
...
PROMPT
PROMPT Privileges Granted to Batch Repository User :
PROMPT Create Table
...
PROMPT Batch Repository User must have access to underlying
PROMPT User data tables to be used in Batch
PROMPT Execute Any Procedure may also be changed, but if
PROMPT Using an Applications EUL the Batch Repository User
PROMPT Must have access to APPS_INITIALIZE()

REM ------------------------------------------------------------------
REM Log in as the Batch Repository User
REM

connect &Batch_User/&Batch_Pass@&Batch_DBLink;

PROMPT
PROMPT Logged in as Batch Repository User
PROMPT

REM ------------------------------------------------------------------
REM Check to see if the package EUL4_BATCH_REPOSITORY exists
...
select 'prompt EUL4_BATCH_REPOSITORY already exists '
from dual
where exists
(select distinct name
 from user_source
 where name = 'EUL4_BATCH_REPOSITORY')
UNION
select 'accept pkgCheck prompt ''Do you wish to replace EUL4_BATCH_REPOSITORY ? [N] '''
from dual
where exists
(select distinct name
 from user_source
...
REM ------------------------------------------------------------------
REM Create the package EUL4_BATCH_REPOSITORY
REM

PROMPT 
PROMPT Creating the package EUL4_BATCH_REPOSITORY ...
PROMPT

set termout off

-- =================================================================================
-- EUL4_BATCH_REPOSITORY SPECIFICATION
-- =================================================================================

CREATE OR REPLACE PACKAGE EUL4_BATCH_REPOSITORY AS
...
END EUL4_BATCH_REPOSITORY;
/
-- =================================================================================
-- EUL4_BATCH_REPOSITORY BODY
-- =================================================================================
CREATE OR REPLACE PACKAGE BODY EUL4_BATCH_REPOSITORY AS
...
BEGIN
    dropSQL := 'DROP PACKAGE EUL4_BATCH_PACKAGE' || timeStamp;
...
BEGIN
    ...
    PACKAGE_SPEC :=
    'CREATE OR REPLACE PACKAGE EUL4_BATCH_PACKAGE' || timeStamp ||
    ' AS'                                                        || chr(10) ||
    '   PROCEDURE RUN;'                                          || chr(10) ||
    'END EUL4_BATCH_PACKAGE' || timeStamp || ';';

    PACKAGE_BODY :=
    'CREATE OR REPLACE PACKAGE BODY EUL4_BATCH_PACKAGE' || timeStamp || ' AS'   || chr(10) ||
    ...
    preExec                                                                    || chr(10) ||
    '   BEGIN'                                                                 || chr(10) ||
    '      IF (EUL4_BATCH_REPOSITORY.IsReportValid(eulSchemaName,'              || chr(10) ||
    '                                          batchReportId) = FALSE) THEN'   || chr(10) ||
    '           RETURN;'                                                       || chr(10) ||
    '      END IF;'                                                            || chr(10) ||
    '      EUL4_BATCH_REPOSITORY.GetUserLimits(eulSchemaName,'                  || chr(10) ||
    '                                      batchReportId,'                     || chr(10) ||
    '                                      userName,'                          || chr(10) ||
    '                                      batchCommitSize,'                   || chr(10) ||
    '                                      rowFetchLimit);'                    || chr(10) ||
    '      EUL4_BATCH_REPOSITORY.SetExpiredRuns(eulSchemaName,'                 || chr(10) ||
    '                                       userName);'                        || chr(10) ||
    '      EUL4_BATCH_REPOSITORY.SetBatchReportRunInProgress(eulSchemaName,'    || chr(10) ||
    '                                                    batchReportId,'       || chr(10) ||
...
BEGIN
    ...
    '      EUL4_BATCH_REPOSITORY.ExecuteQuery(eulSchemaName,'                || chr(10) ||
    '                                     timeStamp,'                       || chr(10) ||
    '                                     batchReportRunId,'                || chr(10) ||
    '                                     batchReportRunNo,'                || chr(10) ||
    '                                     userName,'                        || chr(10) ||
...
BEGIN
    ...
    '         error := TRUE;'                                               || chr(10) ||
    '   END;'                                                               || chr(10) ||
    '   EUL4_BATCH_REPOSITORY.ScheduleRun(eulSchemaName,'                    || chr(10) ||
    '                                 timeStamp,'                           || chr(10) ||
    '                                 batchReportId,'                       || chr(10) ||
    '                                 batchReportRunId,'                    || chr(10) ||
    '                                 batchReportRunNo,'                    || chr(10) ||
    ...
    '                                 startDate);'                          || chr(10) ||
    'END RUN;'                                                              || chr(10) ||
    'END EUL4_BATCH_PACKAGE' || timeStamp || ';'                             || chr(10);
...
BEGIN
    ...
    sqlEULSelect := 'SELECT ver_name FROM <EUL_SCHEMA>.eul4_versions';
...
REM -----------------------------------------------------------------
REM Grant Public access to this Package
REM

set termout on

PROMPT
PROMPT Granting access to EUL4_BATCH_REPOSITORY for all users
PROMPT
set termout off

grant execute on EUL4_BATCH_REPOSITORY to PUBLIC;

REM -----------------------------------------------------------------
REM Grant Batch Repository User access to EUL
REM

set termout on

PROMPT
PROMPT Granting Batch Repository User access to EUL
PROMPT
PROMPT A Batch Repository User can only have access to one EUL
PROMPT
...
PROMPT
PROMPT Connected to EUL, granting access to Batch Repository User ...
PROMPT

set termout off

grant select, insert, update, delete on EUL4_ACCESS_PRIVS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_APP_PARAMS to &Batch_User;                                                                                                                                 
grant select, insert, update, delete on EUL4_ASMP_CONS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_ASMP_LOGS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_ASM_POLICIES to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_BAS to &Batch_User;                                                                                                                                        
grant select, insert, update, delete on EUL4_BATCH_PARAMS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_BATCH_QUERIES to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_BATCH_REPORTS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_BATCH_SHEETS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_BA_OBJ_LINKS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_BQ_DEPS to &Batch_User;                                                                                                                                    
grant select, insert, update, delete on EUL4_BQ_TABLES to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_BR_RUNS to &Batch_User;                                                                                                                                    
grant select, insert, update, delete on EUL4_DBH_NODES to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_DOCUMENTS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_DOMAINS to &Batch_User;                                                                                                                                    
grant select, insert, update, delete on EUL4_ELEM_XREFS to &Batch_User;                                                                                                                                 
grant select, insert, update, delete on EUL4_EUL_USERS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_EXPRESSIONS to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_EXP_DEPS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_FREQ_UNITS to &Batch_User;                                                                                                                                 
grant select, insert, update, delete on EUL4_FUNCTIONS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_FUN_ARGUMENTS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_FUN_CTGS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_FUN_FC_LINKS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_GATEWAYS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_HIERARCHIES to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_HI_NODES to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_HI_SEGMENTS to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_IG_EXP_LINKS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_IHS_FK_LINKS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_KEY_CONS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_OBJS to &Batch_User;                                                                                                                                       
grant select, insert, update, delete on EUL4_OBJ_DEPS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_OBJ_JOIN_USGS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_PLAN_TABLE to &Batch_User;                                                                                                                                 
grant select, insert, update, delete on EUL4_QPP_STATS to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_SEGMENTS to &Batch_User;                                                                                                                                   
grant select, insert, update, delete on EUL4_SEQUENCES to &Batch_User;                                                                                                                                  
grant select, insert, update, delete on EUL4_SQ_CRRLTNS to &Batch_User;                                                                                                                                 
grant select, insert, update, delete on EUL4_SUB_QUERIES to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_SUMMARY_OBJS to &Batch_User;                                                                                                                               
grant select, insert, update, delete on EUL4_SUMO_EXP_USGS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_SUM_BITMAPS to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_SUM_RFSH_SETS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_VERSIONS to &Batch_User;                                                                                                                                   

grant select, insert, update, delete on EUL4_NAMED_ELEMS to &Batch_User;                                                                                                                                
grant select, insert, update, delete on EUL4_ODBC_CATALOGS to &Batch_User;                                                                                                                              
grant select, insert, update, delete on EUL4_ODBC_SCHEMAS to &Batch_User;                                                                                                                               

grant select on EUL4_ID_SEQ to &Batch_User;                                                                                                                                                             

grant execute on eul4_get_object_name to &Batch_User;                                                                                                                                                   

grant execute on eul4_get_item_name to &Batch_User;                                                                                                                                                     

REM -----------------------------------------------------------------
REM Installation of Batch Repository User Complete
REM

set termout on

PROMPT 
PROMPT Batch Repository User Installation Process Finished
PROMPT
