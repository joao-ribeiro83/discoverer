...
export function transformHierarchy(hierarchy: Hierarchy, _version: EulVersion): TransformedHierarchy {
  ...
  if (hierarchy.spannedBusinessAreaIds.length > 1) {
    warnings.push({
      code: 'HIER_SPANS_BUSINESS_AREAS',
      message:
        `Hierarchy "${hierarchy.name}" reaches business areas ` +
        `${hierarchy.spannedBusinessAreaIds.join(', ')}; Neo allows one, so the root ` +
        `level's business area (${hierarchy.businessAreaId}) was used.`,
      sourceId: hierarchy.sourceId,
    });
  }

  // D-074. A date hierarchy is a template - four fixed levels (Year, Quarter,
  // Month, Day) stamped onto any date item - and Discoverer auto-generates one
  // instantiation per date column. Neither is an authored drill path, so
  // neither is imported: Neo regenerates the date drill from the date item
  // itself. The templates' own shape is asserted by the reader's tests, so a
  // template that is NOT the standard four levels shows up as a failure rather
  // than vanishing into this skip.
  let skipReason: HierarchySkipReason | null = null;
  if (hierarchy.hierarchyType === 'DBH') {
    skipReason = 'DATE_TEMPLATE';
    warnings.push({
      code: 'HIER_DATE_TEMPLATE',
      message:
        `Hierarchy "${hierarchy.name}" is a date-hierarchy template ` +
        `(${hierarchy.dateTemplateLevels.map((l) => l.name).join(' > ') || 'no levels'}); ` +
        `Discoverer Neo regenerates date drill paths, so it was not imported.`,
      sourceId: hierarchy.sourceId,
    });
  } else if (hierarchy.sysGenerated) {
    skipReason = 'SYSTEM_GENERATED';
    warnings.push({
      code: 'HIER_SYSTEM_GENERATED',
      message:
        `Hierarchy "${hierarchy.name}" was generated by Discoverer` +
        (hierarchy.fromDateTemplateId !== null
          ? ` from date template ${hierarchy.fromDateTemplateId}`
          : '') +
        `, not authored; it was not imported.`,
      sourceId: hierarchy.sourceId,
    });
  } else if (hierarchy.businessAreaId === null) {
    skipReason = 'NO_BUSINESS_AREA';
    warnings.push({
      code: 'HIER_NO_BUSINESS_AREA',
      message:
        `Hierarchy "${hierarchy.name}" has no level whose item resolves to a business ` +
        `area through IG_EXP_LINKS -> EXPRESSIONS -> BA_OBJ_LINKS; skipped.`,
      sourceId: hierarchy.sourceId,
    });
  }
...
export function transformGrant(grant: Grant, _version: EulVersion): TransformedGrant {
  ...
  if (grant.level === 'BUSINESS_AREA' && grant.privLevel !== null && grant.privLevel !== 0) {
    warnings.push({
      ...
      message:
        `Grant ${grant.sourceId} for "${grant.grantee}" has AP_PRIV_LEVEL ` +
        `${grant.privLevel}, whose meaning is undocumented in every Oracle source ` +
        `available; it was migrated as VIEW (the narrowest level) and needs review.`,
