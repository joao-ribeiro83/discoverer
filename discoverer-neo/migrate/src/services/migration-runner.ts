...
import type { HierarchySkipReason } from './transformers/index.js';
...
export async function runMigration(options: RunMigrationOptions): Promise<MigrationResult> {
  ...
  // Why a hierarchy is not migrated, in the reconciliation's words. D-074: a
  // date-hierarchy template and the instances Discoverer stamps from it are
  // regenerated by Neo, not imported.
  const HIERARCHY_SKIP_REASONS: Record<HierarchySkipReason, string> = {
    DATE_TEMPLATE: 'date-hierarchy template — regenerated natively, not imported (D-074)',
    SYSTEM_GENERATED: 'system-generated date-hierarchy instance — regenerated natively (D-074)',
    NO_BUSINESS_AREA: 'no level resolves a business area',
  };
  ...
  for (const eulHier of eul.data.hierarchies) {
    ...
    collect(t.warnings);
    if (t.skip) {
      const reason = HIERARCHY_SKIP_REASONS[t.skipReason ?? 'NO_BUSINESS_AREA'] ?? 'skipped';
      skipped.push({ table: 'hierarchies', sourceId: t.sourceId, reason });
      await emit('WARN', 'hierarchies', `Hierarchy "${t.name}" (${t.sourceId}) skipped: ${reason}.`, t.sourceId);
      continue;
    }
    const baId = t.businessAreaSourceId !== null ? baIdBySource.get(t.businessAreaSourceId) : undefined;
    if (!baId) {
      skipped.push({ table: 'hierarchies', sourceId: t.sourceId, reason: 'business area not migrated' });
      await emit('WARN', 'hierarchies', `Hierarchy "${t.name}" (${t.sourceId}) skipped: business area not migrated.`, t.sourceId);
      continue;
    }
    const id = deps.genId();
