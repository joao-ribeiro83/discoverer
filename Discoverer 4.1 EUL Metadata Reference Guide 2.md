...
## Appendix A — EUL4 Table Inventory: Export Workflow & Provisional Catalog

### A.1 How this inventory was produced

Oracle never published a complete EUL4 DDL catalog — a 2009 Oracle Community thread asking for "documentation on **all** the EUL 4 tables" (e.g. `EUL4_Qpp_Stats`, `EUL4_Access_Privs`, `EUL4_Objs`) was never authoritatively answered, and no public `CREATE TABLE` dump of an EUL4 schema exists. ([Oracle Community — EUL4 Table Documentation](https://forums.oracle.com/ords/apexds/post/eul4-table-documentation-9908))

Because I could not obtain a real EUL schema export in this session (no Oracle database is connected), the table list in A.2 below is a **provisional catalog** to validate against an export — not a verified, export-derived inventory. A verified inventory requires a real EUL schema export or the output log of the script in A.1. The companion script **`eul4_schema_export.sql`** produces exactly that: it queries `ALL_TABLES`, `ALL_TAB_COLUMNS`, `ALL_CONSTRAINTS`/`ALL_CONS_COLUMNS`, `ALL_INDEXES`, `ALL_OBJECTS`, and (optionally, Oracle 9i+) `DBMS_METADATA.GET_DDL` against your EUL owner (e.g. `EUL4_US`), and emits:

1. every `EUL4_` table and row count,
2. every column of every table (name, datatype, length, nullability),
3. every primary-key column, one row per column ordered by position (`constraint_type = 'P'`),
4. every foreign key actually declared (`constraint_type = 'R'`) — plus a reminder that most EUL joins are *logical*, not physical,
5. every index,
6. the logical parent→child join columns Discoverer itself uses,
7. the non-table objects (sequences, views, functions, packages) that take the total object count toward ~50,
8. (optional) the full `CREATE TABLE` DDL dump.

The script is written for old Oracle releases a Discoverer 4.1 site may still run (8i/9i/10g/11g): it uses no `LISTAGG` and old-style comma joins so it runs even where analytic/ANSI syntax is unavailable. Run it as the EUL owner: `sqlplus <eul_owner>/<pwd>@<tns> @eul4_schema_export.sql`.

### A.2 Provisional base-table catalog (validate against an export)

The table below is the **provisional** EUL4 base-table set — a starting catalog to confirm against the output of `eul4_schema_export.sql`. It is **not** a verified complete list pulled from `ALL_TABLES`. Confidence follows the [Section 3 legend](#3-table-catalog-by-domain): ✅ confirmed from cited sources; ✅* EUL5-confirmed / likely EUL4 analogue; ◻️ conventional Discoverer name for a domain Oracle confirms exists — the export script will verify whether each actually exists and under what exact name in your EUL. **No public source lists all ~23 base tables verbatim; treat this as a starting catalog, not a verified export.**

| # | Table | Domain | Primary key (logical) | Logical FK parent(s) | Conf. |
|---:|---|---|---|---|---:|
| 1 | `EUL4_BAS` | Business areas | `BA_ID` | — | ✅ |
| 2 | `EUL4_BA_OBJ_LINKS` | BA↔Folder links | `BOL_BA_ID, BOL_OBJ_ID` | `EUL4_BAS`, `EUL4_OBJS` | ✅ |
| 3 | `EUL4_OBJS` | Folders / objects | `OBJ_ID` | — | ✅ |
| 4 | `EUL4_EXPRESSIONS` | Items / conditions / calculations | `EXP_ID` | `EUL4_OBJS` (`IT_OBJ_ID`) | ✅ |
| 5 | `EUL4_KEY_CONS` | Joins | `KEY_ID` (probable) | `EUL4_OBJS` (`KEY_OBJ_ID`, `FK_OBJ_ID_REMOTE`) | ✅* |
| 6 | `EUL4_KEY_USAGES` | Join-path usages | — | `EUL4_KEY_CONS`, `EUL4_OBJS` | ◻️ |
| 7 | `EUL4_HIERARCHIES` | Item hierarchies / drill paths | — | `EUL4_EXPRESSIONS` | ◻️ |
| 8 | `EUL4_ITEM_CLASSES` | Item classes / LOVs | — | `EUL4_EXPRESSIONS` | ◻️ |
| 9 | `EUL4_FUNCTS` | Registered PL/SQL functions | — | — | ◻️ |
| 10 | `EUL4_DOCUMENTS` | Workbooks | `DOC_ID` | `EUL4_EUL_USERS` (`DOC_EU_ID`) | ✅* |
| 11 | `EUL4_SHEETS` | Worksheets | — | `EUL4_DOCUMENTS` | ◻️ |
| 12 | `EUL4_ELEM_XREFS` | Worksheet↔item cross-refs | — | `EUL4_EXPRESSIONS` (`EX_TO_ID`); `EUL4_DOCUMENTS` (`EX_FROM_ID`, commonly observed / version-dependent) | ✅* |
| 13 | `EUL4_EUL_USERS` | End users / responsibilities | `EU_ID` | — | ✅* |
| 14 | `EUL4_ACCESS_PRIVS` | Privilege grants / workbook sharing | — | `EUL4_EUL_USERS` (`AP_EU_ID`), `EUL4_DOCUMENTS` (`GD_DOC_ID`) | ✅* |
| 15 | `EUL4_EUL_PRIVS` | EUL-level access | — | `EUL4_EUL_USERS` | ◻️ |
| 16 | `EUL4_ADMIN_PRIVS` | Administrator privileges | — | `EUL4_EUL_USERS` | ◻️ |
| 17 | `EUL4_SUMMARIES` | Summary folders (ASM) | — | `EUL4_OBJS` | ◻️ |
| 18 | `EUL4_QPP_STATS` | Query prediction / run stats | — | (keyed by `QS_DOC_NAME`/`QS_DOC_OWNER`) | ✅ |
| 19 | `EUL4_QSP_STATS` | Summary-refresh / query stats | — | — | ◻️ |
| 20 | `EUL4_PROPERTIES` | Extended / custom properties | — | EUL objects | ◻️ |
| 21 | `EUL4_JOURNAL` | EUL change audit (optional) | — | — | ◻️ |
| 22 | `EUL4_SES` | User sessions | — | `EUL4_EUL_USERS` | ◻️ |
| 23 | `EUL4_VERSION` | EUL structure version | — | — | ◻️ |

### A.3 Non-table objects that take the count toward ≈50

A schema export also returns non-table `EUL4_` objects (if present in your installation). These are the reason the whole EUL object set is commonly quoted as "≈50" rather than the ~23 base tables above:

- **Sequences** — the EUL ID generator producing `EXP_ID`/`OBJ_ID`/`DOC_ID` values. Sequence names vary by installation; read them from `ALL_OBJECTS` via section 7 of the export script.
- **Views** — convenience views over the base tables, if present (used by the EUL Status Workbooks).
- **PL/SQL functions** — the `EUL4_GET_*` helpers created/registered by `eul4.sql`: `EUL4_GET_ITEM`, `EUL4_GET_ITEM_NAME`, `EUL4_GET_OBJECT`, `EUL4_GET_OBJECT_NAME`, `EUL4_GET_SIMPLE_FOLDER`, `EUL4_GET_COMPLEX_FOLDER`, `EUL4_GET_HEIRLVL`, `EUL4_GET_HIERORD`, `EUL4_GET_ANALYZED`, and (Oracle Applications mode) `EUL4_GET_APPS_USERRESP`, `EUL4_GET_ISITAPPS_EUL`. ([Oracle — EUL Status Workbooks](https://docs.oracle.com/html/A86730_01/eul_stat.htm), [Oracle — Using SQL files](https://docs.oracle.com/html/B10270_01/file_lis.htm))
- **Gateway objects** — table/view/registration templates created by `eulgwtbl.sql`, `eulgwvue.sql`, and `eulgwreg.sql`; summary-management privileges are granted by `eulasm.sql`. ([Oracle — Using SQL files](https://docs.oracle.com/html/B10270_01/file_lis.htm))

### A.4 Standard vs Oracle Applications mode

The exact object set depends on EUL mode. **Standard mode** gives the base set above. **Oracle Applications mode** adds Applications-user/responsibility plumbing (the `#<responsibility_id>` encoding seen in `EUL4_EUL_USERS.EU_USERNAME` and the `EUL4_GET_APPS_*` functions). Run `eul4_schema_export.sql` against *your* EUL owner to see which of the ◻️ objects actually exist in your installation.

### A.5 Want the verified inventory for YOUR EUL?

A verified, complete inventory requires a real EUL schema export. Send me an exported schema, DDL, or the output of `eul4_schema_export.sql` (`eul4_inventory.log`) and I'll replace this provisional appendix with a verified, export-derived inventory — exact table names, columns, PKs, and FKs for your installation.
...
- [Oracle Community — EUL4 Table Documentation (unanswered request for full table list)](https://forums.oracle.com/ords/apexds/post/eul4-table-documentation-9908)
