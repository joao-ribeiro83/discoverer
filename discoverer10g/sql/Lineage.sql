-- Lineage.sql
--
-- Before you can use the Lineage.eex Business Area and its workbook you need to run this script
...
-- -----------------    --------------------------------------------------  ------------
--  MJTAYLOR 		Created							20-OCT-01
--  MJTAYLOR		Updated post review by Mark Van de Wiel			02-JAN-02
--  MJTAYLOR		It now automtically Hosts to your browser with URL	11-JAN-02
--  MJTAYLOR		Creates Logfile (lineage.log)				14-JAN-02
--
--
--
set feedback off
set verify off
undefine OWB_URL 
undefine OWB_VIRTUAL_PATH
undefine OWB_DAD
undefine OWB_BROWSER_SCHEMA
undefine OWB_HOST 
undefine OWB_SID 
undefine OWB_SCHEMA 
--
...
Prompt
Prompt Note.
Prompt i.   Before you can use the Lineage.eex Business Area and its workbook you need to run this script.
Prompt
Prompt ii.  Run this SQLPLUS script as the Oracle Discoverer EUL Owner. 
Prompt
Prompt iii. This script needs to be run at least once. It can be rerun as many times as you like.
Prompt
prompt  
prompt   
Prompt You will now be asked to Enter Seven values:
Prompt  1.OWB URL Machine Name.Domain Name:Port     	               eg. http://myserver.us.mycompany.com:7777
Prompt  2.OWB Virtual path to mod_plsql (default=pls)	               eg. pls
Prompt  3.OWB Database Access Descriptor Name - DAD (default=portal30) eg. portal30
Prompt  4.OWB Browser Schema                                           eg. myowbinstall
Prompt  5.OWB Host (Exactly the same as OWB Browser registration name) eg. myserver 
Prompt  6.OWB Sid (Database Instance name)                             eg. 9iASdB
Prompt  7.OWB Schema name where the OWB repository resides             eg. owbrep
Prompt
Prompt
Prompt It will then verify the values you entered by using them to try and access the OWB url. 
Prompt (Log into Portal first, otherwise you will be presented with Username\Password dialog.)  
Prompt 
Prompt
--
--
...
--
--
-- Remove any old records first if they exist
delete from eul5_app_params where app_id in (1019,1020,1021,1022,1023)
/
--
--
accept OWB_URL  prompt '1. Enter http:\\MachineName.DomainName:Port : '
accept OWB_VIRTUAL_PATH prompt '2. Enter Virtual path to mod_plsql [pls] : '
accept OWB_DAD prompt '3. Enter Database Access Descriptor Name - DAD [portal30] : '
accept OWB_BROWSER_SCHEMA prompt '4. Enter Browser Schema : '
accept OWB_HOST prompt '5. Enter Host (OWB Browser registration name) : ' 
accept OWB_SID prompt '6. Enter Sid (Database Instance name) : '
accept OWB_SCHEMA prompt '7. Enter Database Schema where the OWB repository resides : '
prompt
--
--
--
...
set termout off
--
select 'Lineage.log created: '||to_char(sysdate,'dd-mon-yyyy HH:MI:SS') from sys.dual
/
Prompt
Prompt
insert into eul5_app_params values(1019,'PRI',2319,2419,null,nvl('&&OWB_URL','http://myserver.us.mycompany.com:7777'),0,USER,SYSDATE,USER,SYSDATE,NULL)
/
commit;
insert into eul5_app_params values(1020,'PRI',2320,2420,null,'/'||nvl('&&OWB_VIRTUAL_PATH','pls')||'/'||nvl('&&OWB_DAD','portal30')||'/'||'&&OWB_BROWSER_SCHEMA',0,USER,SYSDATE,USER,SYSDATE,NULL)
/
commit;
insert into eul5_app_params values(1021,'PRI',2321,2421,null,nvl('&&OWB_HOST','myserver'),0,USER,SYSDATE,USER,SYSDATE,NULL)
/
commit;
insert into eul5_app_params values(1022,'PRI',2322,2422,null,nvl('&&OWB_SID','9iASdB'),0,USER,SYSDATE,USER,SYSDATE,NULL)
/
commit;
insert into eul5_app_params values(1023,'PRI',2323,2423,null,nvl('&&OWB_SCHEMA','owbrep'),0,USER,SYSDATE,USER,SYSDATE,NULL)
/
commit;
Prompt
--
...
/
set termout on
Prompt Script Running Please wait...
set termout off
--
--
--
--This is a direct copy of EUL5.sql 
--
--
--
--
...
Prompt
--
select 'End of the Logfile at: '||to_char(sysdate,'dd-mon-yyyy HH:MI:SS') from sys.dual
/
--
spool off
set termout on
...
Prompt 
Prompt
Prompt Check the OWB Base URL below.
Prompt
column url format A99 wrap
select nvl('&OWB_URL','http://myserver.us.mycompany.com:7777')||'/'||nvl('&OWB_VIRTUAL_PATH','pls')||'/'||nvl('&OWB_DAD','portal30')||'/'||nvl('&OWB_BROWSER_SCHEMA','myowbinstall')||'.wmp_api_call.show_navigation_page?p_repos_host='||nvl('&OWB_HOST','myserver')||chr(38)||'p_repos_sid='||nvl('&OWB_SID','9iASdB')||chr(38)||'p_repos_schema='||nvl('&OWB_SCHEMA','owbrep')||chr(38)||'p_role=wmp_user'||chr(38)||'p_sheet=4'||chr(38)||'p_object_id=2505'
from dual
/
set termout off
spool Lineage.url
prompt [DEFAULT]
set linesize 2000
column url format A2000 wrap 
select 'BASEURL='||nvl('&OWB_URL','http://myserver.us.mycompany.com:7777')||'/'||nvl('&OWB_VIRTUAL_PATH','pls')||'/'||nvl('&OWB_DAD','portal30')||'/'||nvl('&OWB_BROWSER_SCHEMA','myowbinstall')||'.wmp_api_call.show_navigation_page?p_repos_host='||nvl('&OWB_HOST','myserver')||chr(38)||'p_repos_sid='||nvl('&OWB_SID','9iASdB')||chr(38)||'p_repos_schema='||nvl('&OWB_SCHEMA','owbrep')||chr(38)||'p_role=wmp_user'||chr(38)||'p_sheet=4'||chr(38)||'p_object_id=2505'
from dual
/
Prompt [InternetShortcut]
column url format A2000 wrap
select 'URL='||nvl('&OWB_URL','http://myserver.us.mycompany.com:7777')||'/'||nvl('&OWB_VIRTUAL_PATH','pls')||'/'||nvl('&OWB_DAD','portal30')||'/'||nvl('&OWB_BROWSER_SCHEMA','myowbinstall')||'.wmp_api_call.show_navigation_page?p_repos_host='||nvl('&OWB_HOST','myserver')||chr(38)||'p_repos_sid='||nvl('&OWB_SID','9iASdB')||chr(38)||'p_repos_schema='||nvl('&OWB_SCHEMA','owbrep')||chr(38)||'p_role=wmp_user'||chr(38)||'p_sheet=4'||chr(38)||'p_object_id=2505'
from dual
/
spool off
set termout on
prompt
pause Press RETURN to automatically verify the OWB URL in your browser
host Lineage.url
Prompt
prompt Finished....
Prompt If any values entered are incorrect, rerun this script (Lineage.sql).
Prompt 
set escape off
set heading on
set verify on
